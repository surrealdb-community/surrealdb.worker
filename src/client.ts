...
export class LiveWrapper<T = any> {
  ...
  start() {
    this.bc.dispatchEvent(new MessageEvent('message', {
      data: {
        id: this.id,
        method: 'live',
        vars: this.args,
...
export type WrapWorkerContent<C extends WorkerContent> = {
  query: {
    [K in keyof C['query']]: (...args: Parameters<C['query'][K]>) => ReturnType<C['query'][K]>
  },
  live: {
    [K in keyof C['live']]: (...args: Parameters<C['live'][K]>) => ReturnType<C['live'][K]> extends Live<infer X> ? LiveWrapper<X> : never
  }
}
...
export function setupClient<C extends WorkerContent>(name = "default"): WrapWorkerContent<C> {
  ...
  mainBC.addEventListener('message', (ev: MessageEvent<{
    type: 'hello'
  }>) => {
    if(ev.data.type === 'hello') {
      // restate client
      mainBC.dispatchEvent(new MessageEvent<StartRequest>('message', {
        data: {
          id,
          type: 'start'
        }
  ...
  const promiseMap = new Map<string, [(res: any) => void, (err: any) => void]>()
      ...
      get(_, p: string) {
        return (...args: Parameters<C["query"][keyof C["query"]]>) => {
          const id = reqId()
            ...
            bc.dispatchEvent(new MessageEvent<QueryRequest<C>>('message', {
              data: {
                id,
                method: 'query',
                vars: args,
